...
from database.postservice import get_exact_post_comments_db, public_comment_db, change_exact_user_comment_db, \
    delete_exact_user_comment_db
...
async def change_exact_user_comment(request: Request):
    ...
    if comment_id and new_comment_text:
        change_exact_user_comment_db(comment_id, new_comment_text)
...
async def delete_exact_user_comment(request: Request):
    ...
    comment_id = data.get("comment_id")
    if comment_id:
        delete_exact_user_comment_db(comment_id)
